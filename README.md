...
It is required to have [`docker`](https://www.docker.com/) and [`docker-compose`](https://docs.docker.com/compose/) (optional) installed.
...
Without docker-compose you need to build the container with `docker build -f docker/Dockerfile -t digital-fuesim-manv .` (in the root directory) after that execute `docker run -p 80:80 digital-fuesim-manv`.
